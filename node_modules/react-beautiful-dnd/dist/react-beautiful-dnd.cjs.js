  ...
  var startCaptureBinding = useMemoOne.useMemo(function () {
    return {
      ...
      fn: function onMouseDown(event) {
        ...
        var draggableId = api.tryGetClosestDraggableId(event);
...
function serialize(options) {
  return _JSON$stringify(options);
}
function deserialize(raw) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    var parsedKeys = _Object$keys(parsed);

    var proposedKeys = _Object$keys(proposed);

    var arrange = function arrange(keys) {
      return keys.sort().join('');
    };

    !(arrange(parsedKeys) === arrange(proposedKeys)) ? process.env.NODE_ENV !== "production" ? invariant(false, "Expected " + arrange(parsedKeys) + " to equal " + arrange(proposedKeys)) : invariant(false) : void 0;
    proposedKeys.forEach(function (key) {
      !(proposed[key] != null) ? process.env.NODE_ENV !== "production" ? invariant(false, "Expected parsed object to have key \"" + key + "\"") : invariant(false) : void 0;
    });
  }
...
function getDataFromDraggable(draggable$1) {
  var id = draggable$1.getAttribute("" + draggable.id);
  !(id != null) ? process.env.NODE_ENV !== "production" ? invariant(false, 'Expected element to be a draggable') : invariant(false) : void 0;
  var options = draggable$1.getAttribute("" + draggable.options);
  !options ? process.env.NODE_ENV !== "production" ? invariant(false, 'Expected draggable to have options') : invariant(false) : void 0;
  return _extends({
    id: id
  }, deserialize(options));
}
...
function tryStart(_ref2) {
  var lockAPI = _ref2.lockAPI,
      contextId = _ref2.contextId,
      store = _ref2.store,
      draggableId = _ref2.draggableId,
      forceSensorStop = _ref2.forceSensorStop,
  ...
  var draggable = tryGetDraggable(contextId, draggableId);
  ...
  var _getDataFromDraggable = getDataFromDraggable(draggable),
      id = _getDataFromDraggable.id,
      isEnabled = _getDataFromDraggable.isEnabled,
      canDragInteractiveElements = _getDataFromDraggable.canDragInteractiveElements,
      shouldRespectForcePress = _getDataFromDraggable.shouldRespectForcePress;

  if (!isEnabled) {
    return null;
  }

  if (sourceEvent && !canDragInteractiveElements && isEventInInteractiveElement(draggable, sourceEvent)) {
    return null;
  }
...
function useSensorMarshal(_ref3) {
  var contextId = _ref3.contextId,
      store = _ref3.store,
      customSensors = _ref3.customSensors,
      enableDefaultSensors = _ref3.enableDefaultSensors;
  ...
  var tryGetClosestDraggableId = useMemoOne.useCallback(function (event) {
    return tryGetClosestDraggableIdFromEvent(contextId, event);
  }, [contextId]);
  var api = useMemoOne.useMemo(function () {
    return {
      tryGetLock: tryGetLock,
      tryGetClosestDraggableId: tryGetClosestDraggableId
    };
...
function App(props) {
  ...
  }, [contextId, dimensionMarshal, focusMarshal, getCanLift, getIsMovementAllowed, liftInstructionId]);
  useSensorMarshal({
    contextId: contextId,
    store: store,
...
function Draggable(props) {
  ...
  var provided = useMemoOne.useMemo(function () {
    ...
    return result;
  }, [canDragInteractiveElements, contextId, dragHandleProps, draggableId, isEnabled, mapped, onMoveEnd, setRef, shouldRespectForcePress]);
...
function getSecondarySelector() {
  ...
  var getProps = function getProps(ownId, draggingId, impact, afterCritical) {
    ...
    if (!displacement) {
      ...
      return getMemoizedProps(_offset, null, true);
    }
